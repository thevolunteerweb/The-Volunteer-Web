...
def volunteertable(request):
    ...
    if(request.method == 'POST'):
        ...
        if len(profile_data['upcoming_requests']) == 0 and len(profile_data['event_upcoming_requests']) == 0 and len(profile_data['project_upcoming_requests']) == 0:
        ...
        if len(profile_data['ongoing_requests']) == 0 and len(profile_data['event_ongoing_requests']) == 0 and len(profile_data['project_ongoing_requests']) == 0:
...
def ngoevent(request):
    ...
    if request.method=='POST':
        ...
        for event in profile_data['events']:
            ...
            event.activity_goal = {}
            event.activity_goal = json.loads(temp)
