...
def general(request):
    ...
    if(request.method == 'POST'):
        ...
        if profile_data['is_user']:
            ...
            profile_data['requests'] = Volunteer_ngo_request.objects.filter(sender = user_id.id, status="Accept")
            if len(profile_data['requests']) == 0:
                profile_data['is_requests'] = False
            for req in profile_data['requests']:
                req.recepient="jdjdj"
            ...
            profile_data['name'] = user_id.first_name
            profile_data['requests'] = Volunteer_ngo_request.objects.filter(recepient = user_id.id)
            if  len(profile_data['requests']) == 0:
                profile_data['is_requests'] = False
            return render_to_response('home/ngo/ngofeed.html', profile_data, context)
        user_form=UserForm()
        return render_to_response('home/home.html', {'user_form':user_form}, context)
